...
#define kDotSize CGSizeMake (gdValue(16),gdValue(16)) //密码点的大小
...
-(void)initShow1{
    ...
    for (int i=0; i<self.num; i++) {
        ...
        ViewBorderRadius(dotView, kDotSize.height/2, 1, UIColorFromRGB(0x999999));
//        dotView.hidden = YES;
    ...
    switch (_showType) {
        case 1:
        {
            ...
            for (int i = 0; i < textField.text.length; i++) {
                UIView*dotView=((UIView *)[self.dotArr objectAtIndex:i]);
                
                dotView.backgroundColor = mainColor;
                ViewBorderRadius(dotView, kDotSize.height/2, 1, UIColorFromRGB(0xffffff));
            }
        }
            break;
        case 2:{
    ...
    if (textField.text.length == self.num) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.2 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            ...
            if (weakSelf.textBlock) {
                weakSelf.textBlock(textField.text);
            }
            });
...
-(BaseTextField*)textF{
    if (!_textF) {
        _textF = [[BaseTextField alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, K_Field_Height)];

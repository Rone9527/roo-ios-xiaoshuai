...
#define BgColor ACColor(255,255,255) //背景色

#define SeparatorColor ACColor(242, 242, 242) //分割线颜色

#define normalImage [self imageWithColor:ACColor(255,255,255)] //按钮普通状态下的图片

#define highImage [self imageWithColor:ACColor(255,255,255)] //按钮高亮状态下的图片
...
- (instancetype)initWithActionSheetTitles: (NSArray<NSString *> *)titles cancelTitle: (NSString *)cancelTitle {
    ...
    CGFloat sheetHeight = (titles.count + 1) * ButtonHeight + CancleMargin+kTabbarSafeBottomMargin;
...
- (void)setupButtonsWithTitles: (NSArray *)titles {
    ...
    for (NSInteger idx = 0; idx < titles.count; idx ++) {
        ...
        UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0,ButtonHeight-0.5, ACScreenWidth, 0.5)];
...
- (void)setupCancelButtonWithTitle: (NSString *)title {
    ...
    CGFloat btnY = self.sheetView.frame.size.height - ButtonHeight-kTabbarSafeBottomMargin;
    ...
    [cancel setTitleColor:ziColor forState:UIControlStateNormal];

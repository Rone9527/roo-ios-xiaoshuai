...
//#ifdef DEBUG // 开发
//static NSString const *BTC_API_URL = @"https://testnet.blockchain.info";
...
{
    BTCMnemonic *mnemonic = [BTCWrapper generateMnemonicPassphrase:nil withPassword:nil];
...
+(void)importMnemonicPassphrase:(NSString *)phrase
                      WithBlock:(void(^)(NSString *private,NSString *address,NSString *words))block{
    
    
    BTCMnemonic *mnemonic = [BTCWrapper generateMnemonicPassphrase:phrase withPassword:nil];
    BTCKeychain *keyPr = mnemonic.keychain;
    BTCKey *key = [keyPr keyWithPath:BTC_BIP44_Path];

...
- (instancetype)init {
    if (self = [super init]) {
        ...
        _enableTouchLine = NO;
        _alphaTouchInputLine =1.0;
        _colorTouchInputLine =mainColor;// [UIColor grayColor];
        _widthTouchInputLine = 3.0;
        ...
        ViewBorderRadius(self, 1, 1, [UIColor whiteColor]);
...
- (void)drawLine {
    ...
    for (int i = 0; i<self.allValues.count; i++) {
        ...
        CGFloat yValue = vHeight - BOTTOM_DASH_SPACE - (value-minOrdinateValue)/valueOfYAxis;
//        NSLog(@"sss--%f  %f  %f",yValue,minOrdinateValue,valueOfYAxis);
...
//        UIView*col=[[UIView alloc]initWithFrame:CGRectMake(0,8, 2,2)];
...
//    self.tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(catchedGestureViewEvent:)];
    self.tapGesture=[[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(catchedGestureViewEvent:)];
    self.tapGesture.minimumPressDuration=0.2f;
...
//    }
    ...
    for (UIView *view in self.dotViews) {
        ...
        if (showingViewIndex == index) {
            ...
            _touchInputLine.frame = CGRectMake(view.frame.origin.x+7,view.bottom , _widthTouchInputLine, self.height-view.bottom-gdValue(20));
            _touchInputLine.alpha = _alphaTouchInputLine;
...
- (void)showPopUpViewWithIndex:(NSInteger)index {
    ...
    } else if (index == self.dotViews.count-1|| index == self.dotViews.count-2) {
        x = x-self.popUpView.frame.size.width/2;
    }
...
//    NSLog(@"sd--%f fg--%f a--%f b--%f",x,y,self.popUpView.left,self.popUpView.right);
    self.popUpView.alpha = 1.0;
}
...
//    NSLog(@"a---%f  %f",realMaxValue,realMinValue);
    //纵坐标取整算法
    realMaxValue = MAX(0.0f, realMaxValue);
    
//    .NSLog(@"s---%f",realMaxValue);
//    realMinValue = MIN(0.0f, realMinValue);
    
//    realMinValue = //MIN(realMinValue-1000, realMinValue);
//    NSLog(@"b---%f  %f",realMaxValue,realMinValue);
    //调整最小值
//    minOrdinateValue =realMinValue; //DOUBLE_VALUE_COMPARE_ZERO(realMinValue) >= 0?[self ajustIntegerValue:realMinValue]:(-1 * [self ajustIntegerValue:ABS(realMinValue)]);
    
    
//    if(realMinValue<=50){
//    NSLog(@"sdds---%f",realMinValue);
        realMinValue = realMinValue*0.95;
//    NSLog(@"sss0------%f",realMinValue);
//
        if(realMinValue<0){
...
//    }
    ...
    CGFloat deltaValue = [self calcIntegerDeltaValue:realMaxValue minValue:realMinValue];
    
//    NSLog(@"sd------%f",deltaValue);
    maxOrdinateValue = minOrdinateValue + (numberOfYAxis * deltaValue);
    
//    NSLog(@"c---%f  %f",maxOrdinateValue,minOrdinateValue);
    
    valueOfYAxis = (maxOrdinateValue - minOrdinateValue)/(vHeight-BOTTOM_DASH_SPACE);
    for (int i = 0; i < numberOfYAxis; i++) {
//        NSLog(@"sd---%@",[NSString stringWithFormat:@"%.1f",(minOrdinateValue + i*deltaValue)]);
        [self.YAxisValues addObject:[NSString stringWithFormat:@"%.1f",(minOrdinateValue + i*deltaValue)]];
    }
}

- (CGFloat) calcIntegerDeltaValue:(double) maxValue minValue:(double) minValue{
    CGFloat integerDetla = (maxValue - minValue) / (numberOfYAxis-1);//ceil((maxValue - minValue) / (numberOfYAxis-1));
//    NSLog(@"s----f--%f %f %f",maxValue - minValue,(maxValue - minValue) / (numberOfYAxis-1),integerDetla);
...
- (CGFloat)ajustIntegerValue:(CGFloat) origalValue{
    if (origalValue < 100) {
        return origalValue;
    }else{
